...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        ...
        {
            DrawJumpArc(MoveStats.MaxWalkSpeed, Color.white, _isFacingRight);
        }

        if (MoveStats.ShowRunJumpArc)
        {
            DrawJumpArc(MoveStats.MaxRunSpeed, Color.red, _isFacingRight);
        }
    }
    ...
    #region Jump Arc
    private void DrawJumpArc(float moveSpeed, Color gizmoColor, bool facingRight)
    {
        if (_feetColl == null || MoveStats == null) return; // Safety check for editor
        ...
        // Use facingRight parameter instead of MoveStats.DrawRight
        if (facingRight)
        {
            speed = moveSpeed;
        }
